...
func (r *Runtime) newRuntimeImpl(c *Container) (runtimeImpl, error) {
	...
	if c.runtimeHandler != "" {
		runtimeHandler, err := r.ValidateRuntimeHandler(c.runtimeHandler)
		if err == nil && runtimeHandler.RuntimeType == "vm" {
...
func (r *Runtime) runtimeImpl(ctrID string) (runtimeImpl, error) {
	impl, ok := r.runtimeImplList[ctrID]
	if !ok {
		return nil, fmt.Errorf("Couldn't find any runtime related to ctr %s", ctrID)
	}
